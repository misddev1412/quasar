  ...
  const renderOrderTimeline = () => {
    const timelineSteps = [
      ...
      },
      {
        status: 'delivered',
        label: 'Delivered',
        date: order.estimatedDelivery || order.orderDate,
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6">
        ...
        {showActions && (
          <div className="flex gap-2 mt-4 md:mt-0">
            ...
            {onReorder && (
              <Button size="sm" color="primary" onPress={() => onReorder(order.id)}>
                Reorder
              </Button>
            )}
          </div>
      ...
      {showActions && (
        <div className="mt-8 flex flex-wrap gap-3">
          {(order.status === 'pending' || order.status === 'processing') && onCancelOrder && (
            <Button variant="flat" color="danger" onPress={() => onCancelOrder(order.id)}>
              Cancel Order
            </Button>
          )}

          {order.status === 'delivered' && onReturnOrder && (
            <Button variant="flat" color="warning" onPress={() => onReturnOrder(order.id)}>
              Return Items
            </Button>
          )}

          {onReorder && (
            <Button color="primary" onPress={() => onReorder(order.id)}>
              Reorder All Items
            </Button>
          )}

          <Link href="/">
            <Button variant="flat">Continue Shopping</Button>
          </Link>
        </div>
      )}
    </div>
