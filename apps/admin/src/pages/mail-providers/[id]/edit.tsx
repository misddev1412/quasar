...
const EditMailProviderPage: React.FC = () => {
  ...
  const { data, isLoading } = trpc.adminMailProvider.getProviderById.useQuery({ id: id! }, { enabled: !!id });
  const formRef = useRef<{ getValues: () => MailProviderFormData }>(null);
  const [isTestingConnection, setIsTestingConnection] = useState(false);
  
  const updateMutation = trpc.adminMailProvider.updateProvider.useMutation({
    onSuccess: () => {
      addToast({
        type: 'success',
        title: t('common.success', 'Success'),
        description: t('mail_providers.update_success', 'Mail provider updated successfully'),
      });
  ...
  return (
    <CreatePageTemplate
      ...
      breadcrumbs={[
        ...
        { label: t('mail_providers.edit', 'Edit Mail Provider') },
      ]}
    >
      <EntityForm<MailProviderFormData>
        ...
        submitButtonText={t('mail_providers.update_provider', 'Update Provider')}
