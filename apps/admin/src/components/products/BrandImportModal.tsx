...
export const BrandImportModal: React.FC<BrandImportModalProps> = ({
  isOpen,
  onClose,
  onImportSuccess,
}) => {
  ...
  const importMutation = (trpc.adminProductBrands as Record<string, any>).importFromExcel.useMutation();
      ...
      if (jobStatus === 'COMPLETED') {
        ...
        if (summary) {
          const descriptionParts = [
            t('brands.import.imported_count', '{{count}} created', { count: summary.imported ?? 0 }),
          ];
          if ((summary.updated ?? 0) > 0) {
            descriptionParts.push(
              t('brands.import.updated_count', '{{count}} updated', { count: summary.updated ?? 0 })
            );
          }
          if ((summary.errors?.length ?? 0) > 0) {
            descriptionParts.push(
              t('brands.import.errors_count', '{{count}} errors', { count: summary.errors?.length ?? 0 })
            );
          }
          ...
          addToast({
            type: 'success',
            title: dryRun
              ? t('brands.import.dry_run_complete', 'Dry-run completed')
              : t('brands.import.success', 'Import completed'),
        ...
        addToast({
          ...
          description: result?.error || t('brands.import.failed', 'Import failed'),
        });
      }
    }
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const result = await (utils.adminProductBrands as Record<string, any>).downloadExcelTemplate.fetch({}) as {
        data: string;
        filename: string;
        mimeType: string;
      };
      ...
      addToast({
        type: 'error',
        title: t('common.error', 'Error'),
        description: t('brands.import.download_failed', 'Failed to download template'),
      });
    } finally {
  ...
  const summaryCards = useMemo(() => {
    ...
    return [
      {
        label: t('brands.import.total_rows', 'Total rows'),
        value: importSummary.totalRows ?? 0,
      },
      {
        label: t('brands.import.imported', 'Imported'),
        value: importSummary.imported ?? 0,
      },
      {
        label: t('brands.import.updated', 'Updated'),
        value: importSummary.updated ?? 0,
      },
      {
        label: t('brands.import.skipped', 'Skipped'),
        value: importSummary.skipped ?? 0,
      },
      {
        label: t('brands.import.duplicates', 'Duplicates'),
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!selectedFile) {
      addToast({
        type: 'error',
        title: t('common.error', 'Error'),
        description: t('brands.import.select_file_error', 'Please select a file to import.'),
      });
      return;
    }
    ...
    try {
      ...
      const message = error?.message || t('brands.import.failed', 'Failed to import brands.');
      addToast({
        type: 'error',
        title: t('common.error', 'Error'),
        description: message,
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <h2 className="text-xl font-semibold">
              {t('brands.import.title', 'Import brands from Excel')}
            </h2>
            <p className="text-sm text-gray-500">
              {t('brands.import.subtitle', 'Upload an Excel (.xlsx) or CSV file exported from Excel to create or update brands in bulk.')}
            </p>
          </div>
          <div className="flex items-center gap-2 self-start sm:self-auto">
            ...
            >
              {t('brands.import.download_template', 'Download template')}
            </Button>
            {!isBusy && (
              <DialogClose
                type="button"
            ...
            <CardContent className="space-y-6 pt-6">
              <div className="space-y-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    {t('brands.import.file_label', 'Select spreadsheet file')} <span className="text-red-500">*</span>
                  </label>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {t('brands.import.supported_formats', 'Supported formats: .xlsx, .xls, .csv')}
                  </p>
                </div>
                ...
                >
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-gray-800 dark:text-gray-100">
                      {selectedFile
                        ? selectedFile.name
                        : t('brands.import.drop_zone', 'Click to choose a file')}
                    </p>
                    {selectedFile ? (
                      <>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        {!isBusy && (
                          <p className="text-xs text-gray-400 dark:text-gray-500">
                            {t('brands.import.replace_hint', 'Click to replace file')}
                          </p>
                        )}
                      </>
                    ) : (
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {t('brands.import.drop_zone_hint', 'Drag and drop your file here or browse from your device.')}
                      </p>
                    )}
                  </div>
                </label>
              ...
              {(status === 'PENDING' || status === 'PROCESSING') && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-center text-gray-500 animate-pulse">
                    {t('brands.import.do_not_close', 'Please wait. This may take a while depending on file size.')}
                  </p>
                </div>
              )}
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="group relative flex items-start justify-between gap-3 rounded-xl border border-gray-200 bg-white px-6 py-6 transition hover:border-primary/60 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-gray-700 dark:bg-gray-900">
                  <div className="max-w-[70%]">
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                      {t('brands.import.override_existing', 'Override existing brands')}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {t('brands.import.override_hint', 'If a brand name already exists, update it instead of skipping.')}
                    </p>
                  </div>
                  <Checkbox
                ...
                <div className="group relative flex items-start justify-between gap-3 rounded-xl border border-gray-200 bg-white px-6 py-6 transition hover:border-primary/60 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-gray-700 dark:bg-gray-900">
                  <div className="max-w-[70%]">
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                      {t('brands.import.dry_run', 'Dry-run (no changes)')}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {t('brands.import.dry_run_hint', 'Validate the file, see the summary, but do not create or update brands.')}
                    </p>
                  </div>
                  <Checkbox
                    checked={dryRun}
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  <p className="font-medium text-gray-800 dark:text-gray-100">
                    {t('brands.import.instructions_title', 'Formatting guidelines')}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {t('brands.import.instructions_subtitle', 'Follow these tips to avoid common formatting issues before you upload.')}
                  </p>
                </div>
                <ul className="list-disc space-y-1 text-xs text-gray-500 dark:text-gray-400 pl-4">
                  <li>{t('brands.import.instructions_row1', 'Each row represents a single brand.')}</li>
                  <li>{t('brands.import.instructions_row2', 'Brand ID is optional for new brands and required for updates.')}</li>
                  <li>{t('brands.import.instructions_row3', 'Set Is Active to true/false to control visibility.')}</li>
                  <li>{t('brands.import.instructions_row4', 'Provide image URLs for the Logo field if available.')}</li>
                  <li>{t('brands.import.instructions_row5', 'Leave optional columns blank if you do not need them.')}</li>
                </ul>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                    {dryRun
                      ? t('brands.import.summary_dry_run', 'Dry-run summary')
                      : t('brands.import.summary_title', 'Import summary')}
                  </h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {importSummary.errors && importSummary.errors.length > 0
                      ? t('brands.import.summary_with_errors', 'Some rows could not be processed. Review the details below.')
                      : t('brands.import.summary_success', 'All rows were processed successfully.')}
                  </p>
                </div>
              </div>
              ...
              {importSummary.errors && importSummary.errors.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-gray-700 dark:text-gray-200">
                    {t('brands.import.error_details', 'First {{count}} errors', { count: Math.min(importSummary.errors.length, 5) })}
                  </p>
                  <ul className="space-y-1 rounded-lg border border-error/20 bg-error/5 p-3 text-xs text-error-600 dark:border-error/30 dark:bg-error/10">
                    {importSummary.errors.slice(0, 5).map((err, index) => (
                      <li key={`${err.row}-${index}`}>
                        {t('brands.import.error_row', 'Row {{row}}: {{message}}', {
                          row: err.row,
                          message: err.message,
                        })}
                      </li>
                  ...
                  {importSummary.errors.length > 5 && (
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {t('brands.import.more_errors', '+{{count}} additional errors not shown.', {
            ...
            {importSummary.details && importSummary.details.length > 0 && (
              <div className="border-t border-gray-200 px-6 py-6 dark:border-gray-700">
                <h3 className="mb-4 text-sm font-semibold text-gray-800 dark:text-gray-100">
                  {t('brands.import.details_title', 'Import details')}
                </h3>
                    ...
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                          {t('brands.import.row', 'Row')}
                        </th>
                        <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                          {t('brands.import.brand_name', 'Brand Name')}
                        </th>
                        <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                          {t('brands.import.status', 'Status')}
                        </th>
                        <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">
                          {t('brands.import.message', 'Message')}
                        </th>
                      </tr>
                    </thead>
                      ...
                      {importSummary.details.map((detail, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          ...
                          <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-900 dark:text-gray-100">
                            {detail.name || <span className="text-gray-400 italic">{t('brands.import.unknown', 'Unknown')}</span>}
                          </td>
        ...
        <footer className="flex justify-end gap-3">
          ...
          >
            {dryRun
              ? t('brands.import.run_validation', 'Validate file')
              : t('brands.import.start_import', 'Start import')}
          </Button>
        </footer>
      </form>
    </Modal>
    ...
    case 'IMPORTED':
      return (
        <span className="inline-flex items-center rounded-full bg-success-50 px-2 py-1 text-xs font-medium text-success-700 ring-1 ring-inset ring-success-600/20 dark:bg-success-400/10 dark:text-success-400 dark:ring-success-400/30">
          {t('brands.import.status_imported', 'Imported')}
        </span>
      );
    case 'UPDATED':
      return (
        <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10 dark:bg-blue-400/10 dark:text-blue-400 dark:ring-blue-400/30">
          {t('brands.import.status_updated', 'Updated')}
        </span>
      );
    case 'SKIPPED':
      return (
        <span className="inline-flex items-center rounded-full bg-yellow-50 px-2 py-1 text-xs font-medium text-yellow-800 ring-1 ring-inset ring-yellow-600/20 dark:bg-yellow-400/10 dark:text-yellow-500 dark:ring-yellow-400/20">
          {t('brands.import.status_skipped', 'Skipped')}
        </span>
      );
    case 'ERROR':
      return (
        <span className="inline-flex items-center rounded-full bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10 dark:bg-red-400/10 dark:text-red-400 dark:ring-red-400/20">
          {t('brands.import.status_error', 'Error')}
        </span>
      );
    default:
      return null;
