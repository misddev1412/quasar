...
export const CreateBrandModal: React.FC<CreateBrandModalProps> = ({
  ...
  const createMutation = trpc.adminProductBrands.create.useMutation({
    onSuccess: async (result: any) => {
      ...
      // Create translations if any exist and if brandId is available
      if (brandId) {
        for (const [locale, translationData] of Object.entries(translations)) {
          if (translationData.name || translationData.description) {
            try {
          ...
          <div className="space-y-4">
            <div className="border-t pt-4">
              ...
              <TranslationTabs
                ...
                entityName={formData.name || t('brands.create', 'Create Brand')}
                fields={[
                  {
                    name: 'name',
