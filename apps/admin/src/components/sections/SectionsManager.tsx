...
const SectionConfigEditor: React.FC<SectionConfigEditorProps> = ({ type, value, onChange }) => {
  ...
  const [brandSelectorValue, setBrandSelectorValue] = useState<string | undefined>();
  ...
  const brandIds = useMemo(
    () =>
      Array.isArray(value?.brandIds)
        ? (value.brandIds as unknown[]).filter((id): id is string => typeof id === 'string' && !!id)
        : [],
    [value?.brandIds],
  );
  ...
  useEffect(() => {
    if (type !== SectionType.BRAND_SHOWCASE) {
      setBrandSelectorValue(undefined);
      setCustomBrandDetails({});
    ...
    (async () => {
      try {
        ...
        const responses = await Promise.all(
          brandIds.map(async (id) => {
            try {
              ...
              const translation = Array.isArray(brand.translations) && brand.translations.length > 0
        ...
        const nextMap: Record<string, CustomBrandSummary> = {};
        responses.forEach((entry) => {
          if (entry?.id) {
            nextMap[entry.id] = entry;
          }
        });
        setCustomBrandDetails(nextMap);
        setBrandLoadError(null);
      } catch (error: any) {
        if (!cancelled) {
          setBrandLoadError(error?.message || 'Failed to load brands');
        }
      } finally {
        if (!cancelled) {
          setIsLoadingBrandDetails(false);
