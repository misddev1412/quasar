...
interface EditDeliveryMethodModalProps {
  isOpen: boolean;
...
export const EditDeliveryMethodModal: React.FC<EditDeliveryMethodModalProps> = ({
  isOpen,
  deliveryMethod,
  onClose,
  onSuccess,
  ...
  // Initialize form data when deliveryMethod changes
  useEffect(() => {
    if (deliveryMethod) {
      setFormData({
        ...
        deliveryCost: deliveryMethod.deliveryCost || 0,
        costCalculationType: deliveryMethod.costCalculationType || 'FIXED',
        freeDeliveryThreshold: deliveryMethod.freeDeliveryThreshold,
        minDeliveryTimeHours: deliveryMethod.minDeliveryTimeHours,
        maxDeliveryTimeHours: deliveryMethod.maxDeliveryTimeHours,
        weightLimitKg: deliveryMethod.weightLimitKg,
        sizeLimitCm: deliveryMethod.sizeLimitCm || '',
        coverageAreas: deliveryMethod.coverageAreas || [],
        providerName: deliveryMethod.providerName || '',
  ...
  const updateMutation = trpc.adminDeliveryMethods.update.useMutation({
    onSuccess: () => {
      addToast({
        title: 'Success',
        description: t('delivery_methods.update_success'),
        type: 'success'
      });
      onSuccess();
    },
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">
              {t('delivery_methods.edit')}
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t('delivery_methods.edit_description')}
            </p>
          </div>
        </div>
      </div>
